...
(function(factory) {
  ...
  $.fn.orgchart = function(options) {
    ...
    switch (options) {
      ...
      case 'getAllData':
          return getAllData.apply(this, Array.prototype.splice.call(arguments, 1));
  ...
  function _loopChart($chart) {
	  var $tr = $chart.find('tr:first');
	  var subObj={};
	  if($tr.find('.node')[0].id==1){return subObj;}//放置彩蛋，最底层用户信息
	  else{
		subObj = { 'id': $tr.find('.node')[0].id,name:$tr.find('.node:eq(0) div.title').text(),zw:$tr.find('.node:eq(0) div.content').text()};
	  }
	  $tr.siblings(':last').eq(0).each(function() {
//		  $tr.siblings(':last').children().each(function() {
		  if (!subObj.children) { subObj.children = []; }
		  subObj.children.push(_loopChart($(this)));
	  });
	  return subObj;
  }

  function getAllData($chart) {//自定义获取全部数据 LRF
	  var $chart = $chart || $(this).find('.orgchart');
	  if (!$chart.find('.node:first')[0].id) {
		  return 'Error: Nodes of orghcart to be exported must have id attribute!';
	  }
	  return _loopChart($chart);
  }
